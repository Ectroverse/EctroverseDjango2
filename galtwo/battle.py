...
def attack_planet(attacking_fleet):
    ...
    attacked_planet = Planets.objects.get(x=attacking_fleet.x, y=attacking_fleet.y, i=attacking_fleet.i)
    ...
    portal_xy_list = Planets.objects.filter(portal=True, owner=defender.user).values_list('x', 'y')
    ...
    if defender.empire == snetwork.empire_holding:
        users = UserStatus.objects.filter(empire=snetwork.empire_holding)
        shields = hpshield
        for u in users:
            shields += (shield_absorb * u.total_shield_networks)
    ...
    if attacker_flee:
        ...
        if attacker.post_attack_order == 5:
            portal_planets = Planets.objects.filter(owner=attacker.id, portal=True)  # should always have at least the home planet, unless razed!!!
