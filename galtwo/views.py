...
def headquarters(request):
    ...
    if request.method == 'POST':
        if request.POST['accept']:
            ...
            if planet and status.fleet_readiness >= -100:
                p = Planets.objects.get(id=planet)
                news = News.objects.filter(planet=planet, news_type='E').first()
                frloss = battleReadinessLoss(status, news.user2.userstatus, p)
                p.owner = status.user
                p.solar_collectors *= 0.8
                p.fission_reactors *= 0.8
                p.mineral_plants *= 0.8
                p.crystal_labs *= 0.8
                p.refinement_stations *= 0.8
                p.cities *= 0.8
                p.research_centers *= 0.8
                p.defense_sats *= 0.8
                p.shield_networks *= 0.8
                p.portal = False
                ...
                p.overbuilt = calc_overbuild(p.size, totbuild)
    ...
    context = {"status": status,
               ...
               "old_news": old_news,
               "news_feed": FeedNews.objects.last(),
...
def scouting(request):
    status = get_object_or_404(UserStatus, user=request.user)
    scouted = Scouting.objects.distinct('planet').filter(user=request.user)
    scouted_list = Scouting.objects.filter(user=request.user, scout__gte=1.0).values_list('planet_id', flat=True)
    user=request.user
    
    if 'checkartis' in request.POST:
        check_id = int(request.POST['user'])
        user = User.objects.get(id=check_id)
        scouted = Scouting.objects.distinct('planet').filter(user=user)
        scouted_list = Scouting.objects.filter(user=user, scout__gte=1.0).values_list('planet_id', flat=True)
    
    order_by = request.GET.get('order_by', 'planet')
    if order_by == 'planet':
        scouted = Scouting.objects.select_related('planet').filter(user=user). \
            order_by('planet__x', 'planet__y', 'planet__i').distinct()

    elif order_by == 'size':
        scouted = Scouting.objects.filter(scout__gte=0.5).select_related('planet').filter(user=request.user).order_by('planet__size').distinct().reverse()
    elif order_by == 'artefact':
        scouted = Scouting.objects.select_related('planet').filter(user=request.user, scout__gte=1.0). \
    ...
    if order_by == 'user':
        scouted = Scouting.objects.select_related('planet').filter(user=request.user).order_by('planet__owner')
    ...
    if 'exp_scouting' in request.POST:
        ...
        for p in planet: 
            ...
            if main_fleet.exploration == 0:
                ...
                Fleet.objects.create(owner=status.user,
                     command_order=10,
                     target_planet=p,
                     x=p.x,
                     y=p.y,
...
def battle(request, fleet_id):
    ...
    attacked_planet = Planets.objects.get(x=fleet.x, y=fleet.y, i=fleet.i)
    
    portal_xy_list = Planets.objects.filter(portal=True, owner=attacked_planet.owner.id).values_list('x', 'y')
            ...
            for i in range(0, len(settings_id)):
                if map_settings[i] == 'PF':
                    ...
                    if not details[i]:
                        ...
                        break
                    try:
                        empire_setting = Empire.objects.get(number=details[i])
                    except:
...
def choose_empire(request):
    status = get_object_or_404(UserStatus, user=request.user)
    error = None
    roundstat = RoundStatus.objects.filter().first()
    roundstat = roundstat.tick_time
    if request.POST and 'faction' in request.POST and 'chose_race' in request.POST and 'chose_emp' in request.POST:
        if request.POST['faction'] == "":
            ...
            if request.POST['chose_emp'] == 'Random':
                empires = Empire.objects.filter(numplayers=0)
                emp_choice = np.random.randint(1, empires.count())
                # empire1 = Empires.objects.get(number=emp_choice)
                empire1 = empires[emp_choice]
            else:
                empire1 = Empire.objects.get(number=int(request.POST['chose_emp']))
                if empire1.numplayers > 0:
                    error = "Empire full!"
                    empire1 = None
            ...
            if empire1 is not None:
                ...
                status.galsel = 2
                status.save()
                for p in Planets.objects.filter(x=empire1.x, y=empire1.y):
                    if p.owner is None:
                        give_first_planet(status.user, status, p)
                        give_first_fleet(Fleet.objects.get(owner=status.user, main_fleet=True))
                        break
                msg = "Hello and welcome to Ectroverse! Please, join us on Discord."
                context = {"msg":msg}
    ...
    context = {"races": races,
               ...
               'roundstat': roundstat,
               'error': error}
    return render(request, "choose_empire.html", context)
...
def map(request, *args):
    ...
    status = get_object_or_404(UserStatus, user=request.user)
    #if status.id == 1:
    #    status = UserStatus.objects.get(id=84)
    ...
    planets = Planets.objects.order_by('x','y','i')
    scouted_planets = Scouting.objects.filter(user=request.user,  scout__gte=1.0).values_list("planet_id", flat=True)
    ...
    for art in artis:
        if status.id == 1:
            ...
            if Scouting.objects.filter(user=request.user,  scout__gte=1.0,  planet=art.on_planet).exists():
                mapgen[art.on_planet.x*10000+art.on_planet.y]["imgarti"]=art.image
                if show_arts == True:
                    mapgen[art.on_planet.x*10000+art.on_planet.y]["show"]="gold"
    context = {"status": status,
...
def planets(request):
    ...
    if order_by == 'planet':
        planets = Planets.objects.filter(owner=request.user).exclude(i=9).order_by('x', 'y', 'i')
    else:
        planets = Planets.objects.filter(owner=request.user).exclude(i=9).order_by(order_by)  # directly use the keyword
    ...
    totsize = 0
    ob = 0
    for planet in Planets.objects.filter(owner=request.user):
    ...
    if request.method=='POST' and 'razeallbuild' in request.POST:
        ...
        for pid in planets_id:
            planet = Planets.objects.get(id=pid)
    ...
    context = {"status": status,
               ...
               "msg": msg,
               "stationed": stationed,
               "a_arts": Planets.objects.filter(owner=request.user).exclude(artefact=None).values_list('id', flat=True),
               "a_sols": Planets.objects.filter(owner=request.user, bonus_solar__gt=0).values_list('id', flat=True),
               "a_minz": Planets.objects.filter(owner=request.user, bonus_mineral__gt=0).values_list('id', flat=True),
               "a_cryz": Planets.objects.filter(owner=request.user, bonus_crystal__gt=0).values_list('id', flat=True),
               "a_ectz": Planets.objects.filter(owner=request.user, bonus_ectrolium__gt=0).values_list('id', flat=True),
               "a_fisz": Planets.objects.filter(owner=request.user, bonus_fission__gt=0).values_list('id', flat=True),
               "a_none": Planets.objects.filter(owner=request.user, artefact=None, bonus_solar=0, bonus_mineral=0, bonus_crystal=0, bonus_ectrolium=0, bonus_fission=0).values_list('id', flat=True),
               "page_title": "Planets"}
...
def planet(request, planet_id, *args):
    status = get_object_or_404(UserStatus, user=request.user)
    planet = get_object_or_404(Planets, pk=planet_id)
    attack_cost = None
    ...
    msg = ''
    news = News.objects.filter(planet=planet, news_type='E')
    exploration_cost = calc_exploration_cost(status)
...
@login_required
@user_passes_test(race_check, login_url="/choose_empire")
def system(request, system_id, *args):
...
def syst(request, system_id, *args):
    ...
    if request.method == 'POST' and 'explore_all' in request.POST:
        ...
        fleets_buffer = Fleet.objects.filter(main_fleet=False, ticks_remaining__lt=1, command_order=10)
        explore_planets(fleets_buffer)
    scouted_planets = Scouting.objects.filter(user=status.user, scout__gte=1.0).values_list("planet_id", flat=True)
    ...
    portal_xy_list = Planets.objects.filter(portal=True, owner=status.user.id).values_list('x', 'y')
    ...
    for building in building_list:
        ...
        if cost_list:  # Remember that cost_list will be None if the tech is too low
            ...
            for i in range(5):  # 4 types of resources plus time
                if i < 4:
                    if building == building_list[9] or building == building_list[8] or building == building_list[7]:
        ...
        if obchecked:    
            ...
            for pid in planets_id:
                ...
                if planet.overbuilt_percent <= obmax:
                    if sc >= totalbuild:
                        building_list_dict[SolarCollector()] = totalbuild
                        totalbuild -= totalbuild
                    else:
                        building_list_dict[SolarCollector()] = sc
                        totalbuild -= sc
                    if fi >= totalbuild:
                        building_list_dict[FissionReactor()] = totalbuild
                        totalbuild -= totalbuild
                    else:
                        building_list_dict[FissionReactor()] = fi
                        totalbuild -= fi
                    if mp >= totalbuild:
                        building_list_dict[MineralPlant()] = totalbuild
                        totalbuild -= totalbuild
                    else:
                        building_list_dict[MineralPlant()] = mp
                        totalbuild -= mp
                    if cl >= totalbuild:
                        building_list_dict[CrystalLab()] = totalbuild
                        totalbuild -= totalbuild
                    else:
                        building_list_dict[CrystalLab()] = cl
                        totalbuild -= cl
                    if rf >= totalbuild:
                        building_list_dict[RefinementStation()] = totalbuild
                        totalbuild -= totalbuild
                    else:
                        building_list_dict[RefinementStation()] = rf
                        totalbuild -= rf
                    if ci >= totalbuild:
                        cur_cities = planet.cities
                        tot_cities = cur_cities + totalbuild
                        building_list_dict[Citie()] = totalbuild
                        totalbuild -= totalbuild
                    else:
                        cur_cities = planet.cities
                        tot_cities = cur_cities + ci
                        building_list_dict[Citie()] = ci
                        totalbuild -= ci
                    if rc >= totalbuild:
                        building_list_dict[ResearchCenter()] = totalbuild
                        totalbuild -= totalbuild
                    else:
                        building_list_dict[ ResearchCenter()] = rc
                        totalbuild -= rc
                    if ds >= totalbuild:
                        building_list_dict[DefenseSat()] = totalbuild
                        totalbuild -= totalbuild
                    else:
                        building_list_dict[DefenseSat()] = ds
                        totalbuild -= ds
                    if sn >= totalbuild:
                        building_list_dict[ShieldNetwork()] = totalbuild
                        totalbuild -= totalbuild
                    else:
                        building_list_dict[ShieldNetwork()] = sn
                        totalbuild -= sn
...
def ranking(request):
    ...
    week = tick_time % 52
    year = tick_time // 52
    no_planets = Planets.objects.all().count()
    ...
    avail_planets = Planets.objects.filter(owner=None).count()
...
def empire_ranking(request):
    ...
    roundstat = RoundStatus.objects.filter().first()
    secs = roundstat.tick_time * roundstat.artetimer
    t_left = datetime.timedelta(seconds=secs)
...
def account(request, player_id):
    ...
    if 'chosen_race' in request.POST:
        ...
        if ground.tick_number < 1 or status.id == 164:
            if 'Rejoin' in request.POST:
                ...
                status.empire_role = ''
                planets = Planets.objects.filter(owner=status.user)
                for p in planets:
                    p.buildings_under_construction = 0
...
def fleets_orders_process(request, *args):
    ...
    if order == 0 or order == 1 or order == 10:  # if attack planet or station on planet, make sure planet exists and get planet object
        ...
        try:
            planet = Planets.objects.get(x=x, y=y, i=i)
    ...
    # join main fleet
    if order == 5:
        portal_planets = Planets.objects.filter(owner=request.user,
...
def fleets(request, *args):
    ...
    if request.method == 'POST' and 'explore_planet' in request.POST:
        try:
            ...
            planet_to_template_explore = Planets.objects.get(id=pl_id)
        except Planet.DoesNotExist:
            planet_to_template_explore = None
    ...
    if request.method == 'POST' and 'attack_planet' in request.POST:
        try:
            ...
            planet_to_template_attack = Planets.objects.get(id=pl_id)
        except Planet.DoesNotExist:
            planet_to_template_attack = None
...
def fleetsend(request):
    ...
    if order == 0 or order == 1 or order == 10:  # if attack planet or station on planet orexplore, make sure planet exists and get planet object
        try:
            planet = Planets.objects.get(x=x, y=y, i=planet_i)
        except Planet.DoesNotExist:
            request.session['error'] = "This planet doesn't exist"
            return fleets(request)
    ...
    speed = travel_speed(status)
    fleet_time = max(0,int(np.floor(min_dist / speed)))  # in ticks
...
def vote(request):
    ...
    try:
        ...
        return HttpResponseRedirect("/galtwo/results")
    ...
    if relation == 'ally':
        if rel is not None and rel.relation_type == 'AO':
            ...
            Relations.objects.create(empire1=current_empire,
                                     ...
                                     relation_remaining_time=rel_time)
    if relation == 'war':
        roundstat = RoundStatus.objects.get(id=1)
        war_declaration_timer = (60 / roundstat.tick_time) * 60
        Relations.objects.create(empire1=current_empire,
                                 empire2=target_empire,
                                 relation_type='W',
...
def pm_options(request):
    ...
    if request.method == 'POST':
        ...
        if request.POST['empire_offer_alliance']:
            ...
            n_extra = None
            t_time = RoundStatus.objects.filter().first()
            t_time = t_time.tick_time
            t_time = 60 / t_time
            if time == "tf":
                time = round(24 * (60*t_time))
                n_extra = "24 hour"
            elif time == "fe":
                time = round(48 * (60*t_time))
                n_extra = "48 hour"
            elif time == "tw":
                time = round(15 * (60*t_time))
                n_extra = "12 hour"
...
def relations(request):
    ...
    if 'accept' in request.POST:
        ...
        t_time = t_time.tick_time
        t_time = 60 / t_time
        n_time = round(rela.relation_length / (60*t_time))
...
def specops(request, *args):
    ...
    if specop_planet_id != None:
        planet_to_template_specop = Planets.objects.get(id=specop_planet_id)
    ...
    if request.method == 'POST' and 'specop_planet' in request.POST:
        try:
            ...
            planet_to_template_specop = Planets.objects.get(id=pl_id)
        except Planet.DoesNotExist:
            planet_to_template_specop = None
    ...
    for g in Ops.objects.filter(specop_type="G"):
        if g.name in inca_ops:
            if g.name == "Sense Artefact" and Artefacts.objects.filter(on_planet__isnull=False).count() == 0:
                ...
                inca[g.name] = [g.tech,b_cost,g.difficulty,g.stealth,g.ident,g.description,fr,tech]
    ...
    if request.method == 'POST':
        ...
        if 'agent_select' in request.POST:
            ...
            for agent_id in agent_select:
                ...
                portal_planets = Planets.objects.filter(owner=request.user, portal=True)
        ...
        if 'ghost_select' in request.POST:
            ...
            for ghost_id in ghost_select:
                ...
                main_fleet = Fleet.objects.get(owner=request.user, main_fleet=True)
                fleets_id3 = Fleet.objects.filter(id=ghost_id, ticks_remaining=0)
                join_main_fleet(main_fleet, fleets_id3)
...
def specop_show(request, specop_id):
    ...
    if specop.name == "High Infiltration":
        if specop.specop_strength >= 1.5:
            planets = Planets.objects.filter(owner=specop.user_to)
            for p in planets:
                scouting = Scouting.objects.filter(planet=p, user=status.user).first()
                if scouting is None:
                    Scouting.objects.create(user=specop.user_from, planet=p, scout=1, empire=status.empire)
...
def search(request):
    ...
    if "!" in u_inp:
        ...
        try:   
            ...
            u_out = Planets.objects.get(x=f_plant[0], y=f_plant_i[0], i=f_plant_i[1])
        ...
        try:
            ...
            return system(request, u_out.id)
        except:
            return fsearch(request)
