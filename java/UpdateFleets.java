...
public class UpdateFleets {
	...
	public void updateMoovingFleets(LinkedList<Planet> portals, int militaryFlag, HashMap<String, Double> race_info) throws Exception{
		...
		ResultSet s_emp = statement.executeQuery("SELECT empire_holding_id FROM app_artefacts WHERE name = 'Blackhole' ");
		s_emp.next();
		...
		if(exploredPlanet.next()){
			...
			if (exploredPlanet.getInt("owner_id") == 0){
				...
				if (! exploredPlanet.wasNull()){
					statement2.execute("UPDATE app_artefacts SET empire_holding_id = " + empireID + " WHERE id = "+ arti + ";");	
				}
