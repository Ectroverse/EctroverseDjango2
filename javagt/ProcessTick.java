...
public class ProcessTick
{
    public static void main(String[] args) {
		...
		try{
			...
			String createSP = "CREATE OR REPLACE PROCEDURE updatePlanet( "
	...
	private final String userStatusUpdateQuery = "UPDATE galtwo_userstatus SET "+
	" fleet_readiness  = ? ," +  //1
	" psychic_readiness  = ? ," + //2
	" agent_readiness  = ? ," + //3
	" population   = ? ," + //4
	...
	private void processTick(Connection con){
		...
		try {
			...
			ResultSet roundCeck = statement.executeQuery("SELECT * FROM galtwo_roundstatus");
		...
		statement.execute("LOCK TABLE \"PLANETS\", galtwo_roundstatus, galtwo_userstatus, galtwo_construction, galtwo_fleet IN ACCESS EXCLUSIVE MODE;");
		
		//update tick number
		ResultSet resultSet = statement.executeQuery("SELECT tick_number FROM galtwo_roundstatus");
		resultSet.next();
		int tick_nr = resultSet.getInt("tick_number");
		System.out.println("Process of tick #" + tick_nr + " has started!");
	
		statement.executeUpdate("UPDATE galtwo_roundstatus SET tick_number = " + (tick_nr + 1) );


		ResultSet TF = statement.executeQuery("SELECT empire_holding_id FROM galtwo_artefacts WHERE name = 'Terraformer' ");
		TF.next();
		int TFA = TF.getInt("empire_holding_id");
		ResultSet TFTF = statement.executeQuery("SELECT ticks_left FROM galtwo_artefacts WHERE name = 'Terraformer' ");
		TFTF.next();
		int TFT = TFTF.getInt("ticks_left");
		ResultSet DU = statement.executeQuery("SELECT empire_holding_id FROM galtwo_artefacts WHERE name = 'Flying Dutchman' ");
		DU.next();
		int FDU = DU.getInt("empire_holding_id");
		ResultSet TFD = statement.executeQuery("SELECT ticks_left FROM galtwo_artefacts WHERE name = 'Flying Dutchman' ");
		TFD.next();
		int FLD = TFD.getInt("ticks_left");
		ResultSet TG = statement.executeQuery("SELECT empire_holding_id FROM galtwo_artefacts WHERE name = 'The General' ");
		TG.next();
		...
		else{statement.execute("UPDATE galtwo_artefacts SET ticks_left = 1 WHERE name = 'Terraformer';");}
		
		if(TFT == 0){
		    ProcessBuilder pb = new ProcessBuilder("python", "/code/manage.py", "generate_terraformer2");
			    Process p = pb.start();
			}
		...
		if(FDU >= 1){
		    statement.execute("UPDATE galtwo_artefacts SET ticks_left = ticks_left - 1 WHERE name = 'Flying Dutchman';");
		    }
		else{statement.execute("UPDATE galtwo_artefacts SET ticks_left = 1 WHERE name = 'Flying Dutchman';");}
		
		if(FLD == 0){
		    ProcessBuilder pb = new ProcessBuilder("python", "/code/manage.py", "generate_dutchman2");
			    Process p = pb.start();
			}
		...
		//udpate rel timer
		statement.execute("UPDATE galtwo_relations SET relation_remaining_time = relation_remaining_time - 1 WHERE relation_type = 'W';");
		statement.execute("UPDATE galtwo_relations SET relation_remaining_time = relation_remaining_time - 1 WHERE relation_type = 'NC';");
		statement.execute("UPDATE galtwo_relations SET relation_remaining_time = relation_remaining_time - 1 WHERE relation_type = 'C';");
		statement.execute("DELETE FROM galtwo_relations WHERE relation_remaining_time = 0;");
		
		//udpate bot attack
		statement.execute("UPDATE galtwo_botattack SET time = time - 1;");
		statement.execute("DELETE FROM galtwo_botattack WHERE time = 0;");
		
		//update specops time
		statement.execute("UPDATE galtwo_specops SET ticks_left = ticks_left - 1;");
		statement.execute("DELETE FROM galtwo_specops WHERE ticks_left = 0;");
		
		resultSet = statement.executeQuery("SELECT * FROM galtwo_userstatus");
		...
		String fleetsDeleteUpdateQuery = "DELETE FROM galtwo_fleet WHERE id = ?";
		...
		String specopIncomeUpdateQuery = "UPDATE galtwo_userstatus SET" +
		...
		for(int j = 0; j < usersInt.size(); j++){
			...
			long population = userLongValues.get("population");
			...
			ResultSet portalstSet = statement3.executeQuery("SELECT * FROM \"PLANETS\" WHERE portal = TRUE AND owner_id = " + userID );
			...
			ResultSet vpportalstSet = statement.executeQuery("SELECT * FROM \"PLANET\" INNER JOIN galtwo_specops ON \"PLANET\".id = planet_id WHERE name = 'Vortex Portal' AND galtwo_specops.user_to_id = " + userID);
			...
			ResultSet operationSet = statement3.executeQuery("SELECT * FROM galtwo_specops WHERE name = 'Enlightenment' AND user_to_id = " + userID);
			...
			ResultSet artefactSet = statement4.executeQuery("SELECT * FROM galtwo_artefacts WHERE name = 'Research Laboratory' AND empire_holding_id = " + empireID);
			...
			artefactSet = statement4.executeQuery("SELECT * FROM galtwo_artefacts WHERE name = 'Ether Gardens' AND empire_holding_id = " + empireID);
			while (artefactSet.next()){
			...
			ResultSet ENG = statement.executeQuery("SELECT empire_holding_id FROM galtwo_artefacts WHERE name = 'Engineer' ");
		    ENG.next();
		    int ENGA = ENG.getInt("empire_holding_id");
		    
		    ResultSet ENS = statement.executeQuery("SELECT empire_holding_id FROM galtwo_artefacts WHERE name = 'Engineers Son' ");
		    ENS.next();
		    int ENSA = ENS.getInt("empire_holding_id");
            
            ResultSet engineerSet = statement4.executeQuery("SELECT * FROM galtwo_artefacts WHERE name = 'Engineer' AND empire_holding_id = " + empireID);
			...
			for(int z = 0; z < unit_names.length; z++){
				...
				String query = "SELECT SUM(" + s + ") FROM galtwo_fleet WHERE owner_id = " + userID;
			...
			long population_upkeep_reduction = population / 350;
			population_upkeep_reduction = Math.min(population_upkeep_reduction, buildings_upkeep + units_upkeep + portals_upkeep);
			
			//Darwinism artefact
			artefactSet = statement4.executeQuery("SELECT * FROM galtwo_artefacts WHERE name = 'Darwinism' AND empire_holding_id = " + empireID);
			while (artefactSet.next()){
				population_upkeep_reduction = population / 315;
			}
			...
			artefactSet = statement4.executeQuery("SELECT * FROM galtwo_artefacts WHERE name = 'Mirny Mine' AND empire_holding_id = " + empireID);
			while (artefactSet.next()){
		    ...
		    //spoil op modifier
			operationSet = statement3.executeQuery("SELECT * FROM galtwo_specops WHERE name = 'Spoil Resources' AND user_to_id = " + userID);
			...
			artefactSet = statement4.executeQuery("SELECT * FROM galtwo_artefacts WHERE name = 'Crystal Synthesis' AND empire_holding_id = " + empireID);
			while (artefactSet.next()){
			...
			artefactSet = statement4.executeQuery("SELECT * FROM galtwo_artefacts WHERE name = 'Crystal Recharger' AND empire_holding_id = " + empireID);
			while (artefactSet.next()){
				crystal_decay *= 0.25;
			}
			...
			artefactSet = statement4.executeQuery("SELECT * FROM galtwo_artefacts WHERE name = 'Foohon Technology' AND empire_holding_id = " + empireID);
			while (artefactSet.next()){
			...
			operationSet = statement3.executeQuery("SELECT * FROM galtwo_specops WHERE name = 'Hack mainframe' AND user_to_id = " + userID);
			...
			if((tick_nr % 2) == 0){
			if( Math.max(0, userLongValues.get("energy") + energy_income) > 0){
			artefactSet = statement4.executeQuery("SELECT * FROM galtwo_artefacts WHERE name = 'Churchills Brandy' AND empire_holding_id = " + empireID);
			while (artefactSet.next()){
			...
			artefactSet = statement4.executeQuery("SELECT * FROM galtwo_artefacts WHERE name = 'The Recycler' AND empire_holding_id = " + empireID);
			while (artefactSet.next()){
		...
		"FROM ( "+
			...
			"FROM galtwo_userstatus GROUP BY empire_id ) AS GroupedUserTable "+
		"WHERE "+
			"galtwo_empire.id = GroupedUserTable.empire_id;";
		statement.execute(updateEmpireRanks);
		
		//purge old news
		String deletePersonalNews = "DELETE FROM galtwo_news WHERE is_personal_news = false AND " + tick_nr + "  - tick_number > " + news_delete_ticks + " ;"; 
		String deleteEmpireNews =  "DELETE FROM galtwo_news WHERE is_personal_news = true AND is_read = true AND "+
		...
		statement.execute("DELETE FROM galtwo_unitconstruction WHERE ticks_remaining = 0;");
