...
public class UpdateFleets {
	...
	private final String fleetStationQuery = "UPDATE galtwo_fleet SET" +
	" bomber = ?" + //1
	" , fighter = ? "+ //2
	" , transport = ? "+ //3
	" , cruiser = ? " + //4
	...
	private final String fleetsDeleteUpdateQuery = "DELETE FROM galtwo_fleet WHERE id = ?";
	
	private final String emptyFleetsDelete = "DELETE FROM galtwo_fleet WHERE " + 
	...
	private final String fleetMoveQuery = "UPDATE galtwo_fleet SET " +
	...
	public void updateFleetBuild() throws Exception{
		...
		String unitsQuery = 
		" SELECT unit_type, SUM(n) as num_units FROM galtwo_unitconstruction "+
	...
	public void updateFleetsMerge() throws Exception{
		ResultSet mergingFleets = statement.executeQuery("SELECT * FROM galtwo_fleet WHERE (command_order = 3 OR command_order = 4) AND owner_id = " + userID + 
		...
		for(Map.Entry<String, LinkedList<Integer>> entry : fleetMarge.entrySet()){
			...
			if (idList.size() > 1){
				...
				for (Integer id : idList) {
					ResultSet fleet = statement.executeQuery("SELECT * FROM galtwo_fleet WHERE id = " + id + ";");
					fleet.next();
					for(int i =0; i < unit_names.length; i++){
						unit[i] += fleet.getLong(unit_names[i]);
					}
					if (id != firstId)
						statement.execute("DELETE FROM galtwo_fleet WHERE id = " + id + ";");
				}
				for(int i =0; i < unit_names.length; i++){
	...
	public void updateExplorationFleets(ResultSet fleet, int militaryFlag) throws Exception{
		ResultSet exploredPlanet = statement3.executeQuery("SELECT * FROM \"PLANETS\" WHERE x = " + fleet.getInt("x") 
		...
		if(exploredPlanet.next()){
			...
			if (exploredPlanet.getInt("owner_id") == 0){
				statement2.execute("UPDATE \"PLANETS\" SET owner_id = " + userID + " WHERE id = "+ planetID + ";");
				//update arti holding family
				int arti = exploredPlanet.getInt("artefact_id");
				if (! exploredPlanet.wasNull()){
					statement2.execute("UPDATE galtwo_artefacts SET empire_holding_id = " + empireID + " WHERE id = "+ arti + ";");	
				}
				...
				statement2.execute("DELETE FROM galtwo_scouting WHERE user_id = " +userID+" AND planet_id = " +planetID+ ";");
				statement2.execute("INSERT INTO galtwo_scouting ( user_id, planet_id, scout)" +
				...
				statement2.execute("UPDATE galtwo_fleet SET ticks_remaining = 0, command_order = 2 WHERE id = " + fleet.getInt("id") +  ";");
	...
	public void updateStationedFleets() throws Exception{
		ResultSet stationingFleets = statement.executeQuery("SELECT * FROM galtwo_fleet WHERE command_order = 1 AND owner_id = " + userID + 
		" AND ticks_remaining = 0;");
		...
		while(stationingFleets.next()){
			...
			ResultSet planet = statement2.executeQuery("SELECT id, owner_id FROM \"PLANETS\" WHERE x = "  +  x +
			" AND y = " + y +
			" AND i = " + i );
			...
			if (planet.next()){
				if (planet.getInt("owner_id") != userID){
					statement2.execute("UPDATE galtwo_fleet SET command_order = 2 WHERE id = " + stationingFleets.getInt("id") + ";");
				...
				statement.execute("UPDATE galtwo_fleet SET command_order = 2 WHERE id = " + stationingFleets.getInt("id") + ";");
		...
		ResultSet phantomsResultSet = statement.executeQuery("SELECT id, phantom FROM galtwo_fleet WHERE owner_id = " + userID + ";");
		ResultSet psychicsResultSet = statement2.executeQuery("SELECT wizard FROM galtwo_fleet WHERE main_fleet = true AND owner_id = " + userID + ";");
