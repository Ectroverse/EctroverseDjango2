...
public class UpdatePlanets{
	...
	private final String planetStatusUpdateQuery = "UPDATE \"PLANETS\"  SET" +
	" protection = ? ," + //1
	" overbuilt = ? ," + //2
	" overbuilt_percent = ? ," + //3
	" solar_collectors = ? ," + //4
	...
	public void updateUserPlanets(int userID, int empireID, HashMap<String,Integer> userIntValues, HashMap<String, Double> race_info,
		...
		ResultSet resultSet = statement.executeQuery("SELECT * FROM \"PLANETS\" WHERE owner_id = " + userID);
		...
		String runSP = "CALL updatePlanet(?, ?, ?); ";
		...
		while(resultSet.next()){
			...
			ResultSet darkMistSet = statement2.executeQuery("SELECT specop_strength FROM galtwo_specops WHERE (name = 'Black Mist' or name = 'Dark Web') "+
	...
	private static HashMap<Integer, HashMap<String, Integer>> execute_jobs (Connection con){
		...
		try{
			String planetjobsUpdateQuery = "UPDATE galtwo_construction SET " +
				" ticks_remaining  = ? " + //1
				" WHERE id = ? " ; //2
			PreparedStatement updateJobsTicks = con.prepareStatement(planetjobsUpdateQuery);	
			
			String deleteJobsQuery = "DELETE FROM galtwo_construction WHERE id = ?";
			PreparedStatement deleteJobs = con.prepareStatement(deleteJobsQuery);	
			
			Statement statement = con.createStatement();
			ResultSet resultSet = statement.executeQuery("SELECT * FROM galtwo_construction");
