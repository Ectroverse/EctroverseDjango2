    ...
    if 'sense_check' in request.POST:    
        ...
        for a in artis:
            ...
            if system.id in sense:
                scout = Scouting.objects.filter(empire=status.empire, planet=p)
                if scout:
...
def research(request):
    ...
    if request.method == 'POST':
        ...
        if 'rc_calc_form' in request.POST:
            ...
            if arte.empire_holding == status.empire:
                rpport = int((race_info.get("research_max_portals") + 100) * (1.0-np.exp(port / (-10.0 * netw))))
...
def specops(request, *args):
    ...
    if alch.empire_holding == status.empire:
        race_spells.append("Alchemist")
    
    for s in psychicop_specs:
        if roundstatus.tick_number < 1008 and s not in off_ops or roundstatus.tick_number >= 1008:
            if s in race_spells:
                specs = [None] * 8
        ...
        if roundstatus.tick_number < 1008 and g not in off_ops or roundstatus.tick_number >= 1008:
            if g in race_inca:
                if g == "Sense Artefact" and Artefacts.objects.filter(on_planet__isnull=False).count() == 0:
                    ...
                    if user_to_template_specop:
                        ig_incs = ["Survey System", "Sense Artefact", "Vortex Portal", "Call to Arms"]
                        if g in ig_incs:
