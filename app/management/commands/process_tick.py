...
class Command(BaseCommand): # must be called command, use file name to name the functionality
    ...
    def handle(self, *args, **options):
        ...
        for status in UserStatus.objects.all():
            ...
            status.fleet_readiness = min(status.fleet_readiness +2, status.fleet_readiness_max)
            status.psychic_readiness  = min(status.fleet_readiness +2, status.psychic_readiness_max)
            status.agent_readiness = min(status.fleet_readiness +2, status.agent_readiness_max)
	        ...
	        */
            ...
            for planet in planets_buffer:
                ...
                if planet.x and planet.y == planet.x and planet.y and planet.portal is True:
                    planet.protection = 100
            ...
            research_percent_military     = int(race_info.get("research_max_military") * (1.0-np.exp(status.research_points_military / (-10.0 * status.networth))))
            research_percent_construction = int(race_info.get("research_max_construction") * (1.0-np.exp(status.research_points_construction / (-10.0 * status.networth))))
            research_percent_tech         = int(race_info.get("research_max_tech") * (1.0-np.exp(status.research_points_tech / (-10.0 * status.networth))))
            research_percent_energy       = int(race_info.get("research_max_energy") * (1.0-np.exp(status.research_points_energy / (-10.0 * status.networth))))
            research_percent_population   = int(race_info.get("research_max_population") * (1.0-np.exp(status.research_points_population / (-10.0 * status.networth))))
            research_percent_culture      = int(race_info.get("research_max_culture") * (1.0-np.exp(status.research_points_culture / (-10.0 * status.networth))))
            research_percent_operations   = int(race_info.get("research_max_operations") * (1.0-np.exp(status.research_points_operations / (-10.0 * status.networth))))
            research_percent_portals      = int(race_info.get("research_max_portals") * (1.0-np.exp(status.research_points_portals / (-10.0 * status.networth))))
            ...
            status.research_percent_military = min(race_info.get("research_max_military"),status.research_percent_military)
            status.research_percent_construction = min(race_info.get("research_max_construction"),status.research_percent_construction)
            status.research_percent_tech = min(race_info.get("research_max_tech"),status.research_percent_tech)
            status.research_percent_energy = min(race_info.get("research_max_energy"),status.research_percent_energy)
            status.research_percent_population = min(race_info.get("research_max_population"),status.research_percent_population )
            status.research_percent_culture = min(race_info.get("research_max_culture"),status.research_percent_culture)
            status.research_percent_operations = min(race_info.get("research_max_operations"),status.research_percent_operations )
            status.research_percent_portals = min(race_info.get("research_max_portals"),status.research_percent_portals)
